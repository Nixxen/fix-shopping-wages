...
// static const ensures the compiler dead-code-eliminates the false branch.
...
void PlayerInterface_updateUT_hook(PlayerInterface *thisPointer)
{
    ...
    if (kVerboseDebugLogging)
    {
        ...
        // --- Hourly money snapshot diff ------------------------------------

        double totalHours = ou->getTimeStamp_inGameHours().getTotalHours();
        int currentHour = static_cast<int>(totalHours);
        if (currentHour != g_previousHour)
        {
            typedef ogre_unordered_set<Character *>::type CharacterSet;
            const CharacterSet &characterList = ou->getCharacterUpdateList();
            ...
            for (CharacterSet::const_iterator iterator = characterList.begin(); iterator != characterList.end();
                 ++iterator)
            {
                Character *character = *iterator;
            ...
            if (g_previousHour != -1)
            {
                ...
                for (std::map<hand, HourlyMoneyEntry>::const_iterator previousIterator =
                         g_previousMoneySnapshot.begin();
                ...
                if (diffCount > 0)
                {
                    ...
                    summary << "[HOURLY DIFF] " << diffCount
                            << " NPC(s) had money changes this hour (hour=" << currentHour << ")";
            ...
            if (selectedCharacter != nullptr)
            {
                ...
                if (characterData != nullptr)
                {
                    ...
                    // -- bool fields (bdata) --
                    {
                        ...
                        for (BoolDataMap::const_iterator iterator = boolMap.begin(); iterator != boolMap.end();
                             ++iterator)
                        {
                            std::stringstream lineMessage;
